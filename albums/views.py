...
from .serializers import AlbumSerializers, AlbumSerializersForPost
...
class AlbumsList(APIView):
    ...
    def post(self, request):
        ...
        request.data["release_datetime"] = timezone.now()
        serializer = AlbumSerializersForPost(data=request.data)
    ...
    def get(self, request):
        ...
        if cost_lte is not None:
            try:
                ...
                return Response(
                    {"error": "Invalid data type for cost__lte filter."},
        ...
        if cost_gte is not None:
            try:
                ...
                return Response(
                    {"error": "Invalid data type for cost__gte filter."},
                    status=status.HTTP_400_BAD_REQUEST,
                )
        if name is not None:
            albums = [album for album in albums if album.name.lower() == name.lower()]
